...
const pluginName = `StandaloneWebWidgetPlugin`;
...
class StandaloneWebWidgetPlugin {
  /**
   *
   * @typedef {{
   * imports: object;
  ...
  modifyScripts({ publicPath, scripts }) {
    ...
    scripts.modifiedByWebWidgetStandalonePlugin = true;
    ...
    const importMap = {
      imports: {
        "web-widget":
          "https://cdn.jsdelivr.net/npm/web-widget/lib/system/web-widget.dev.js",
  ...
  applicationRuntime() {
    return `
      System.import('web-widget').then(function (webWidget) {
        webWidget.registerApplication({
        ...
        if (!window.location.pathname.startsWith('${
          ...
          webWidget.navigateToUrl('${this.options.activeWhen[0]}');
        }
        webWidget.start(${JSON.stringify(this.options.startOptions)});
      });
    `
      .trim()
      .replace(/\n      /g, "\n");
  ...
  parcelRuntime() {
    return `
      Promise.all([System.import('web-widget'), System.import('${this.options.appOrParcelName}')]).then(function (values) {
        const webWidget = values[0];
        ...
        webWidget.mountRootParcel(parcelConfig, { domElement: parcelContainer, name: '${this.options.appOrParcelName}' });
      });
    `;
  }
    ...
    return `
      ...
      <p>
        ...
        <ol>
          ...
          <li>
            In a new browser tab, go to the your web-widget web app. This is where your "root config" is running. You do not have to run the root config locally
      ...
      <p>
        ...
        <ul>
          <li>
            <a target="_blank" rel="noopener" href="https://web-widget.js.org/docs/recommended-setup#local-development">
...
module.exports = StandaloneWebWidgetPlugin;
