...
module.exports = class WebWidgetRootConfigGenerator extends PnpmGenerator {
  ...
  async createPackageJson() {
    const answers = await this.prompt([
      ...
      },
      {
        type: "confirm",
        name: "layout",
        message: "Would you like to use web-widget Layout Engine",
        default: true,
  ...
  async copyFiles() {
    ...
    if (this.options.typescript) {
      ...
      // Will be replaced by webpack-config-web-widget-ts
      delete packageJson.devDependencies[
        "@web-widget/webpack-config-web-widget"
      ];
...
Run '${this.options.packageManager} start' to boot up your web-widget root config`)
      );

      if (this.options.layout) {
        console.log(
          `\nPlease report web-widget-layout issues and bugs on GitHub https://github.com/web-widget/web-widget-layout/issues/new`
        );
      }
    });
  }
