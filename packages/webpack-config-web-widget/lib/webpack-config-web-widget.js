...
module.exports = webpackConfigWebWidget;

function webpackConfigWebWidget(opts) {
  if (typeof opts !== "object") {
    throw Error(`webpack-config-web-widget requires an opts object`);
  }

  if (typeof opts.orgName !== "string") {
    throw Error(`webpack-config-web-widget requires an opts.orgName string`);
  }

  if (typeof opts.projectName !== "string") {
    throw Error(
      `webpack-config-web-widget requires an opts.projectName string`
    );
  }
  ...
  return {
    ...
    externals: opts.orgPackagesAsExternal
      ? ["web-widget", new RegExp(`^@${opts.orgName}/`)]
      : ["web-widget"],
    plugins: [
